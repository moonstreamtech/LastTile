...
- **Hazards:** Fire / Ice / Poison tile types are live. All three freeze the tile they sit on. Ice expires after 3 turns; poison after 7 turns and halves the underlying value (poison whose value would drop below 2 simply releases an empty cell on expiry); fire never dies on its own — if the player can't trigger its cleanse it jumps to the most valuable pending merge every 5 turns. Cleanse is global (board-wide): a 16/32/64 merge clears the oldest ice/fire/poison; a 128 merge wipes every hazard at once.
- **Spawn semantics:** every successful action (merge, slide, swap, split) produces exactly one **mutually exclusive** spawn outcome — either a Normal(2) lands in a random empty cell, or a hazard (Fire/Ice/Poison) infects an existing Normal tile and inherits its value. The two outcomes never coexist on the same turn. Hazard target selection prefers a Normal that has a same-value neighbour (the player's pending merge), then the highest-value tile in that pool. Per-kind cap and 3-turn post-death cooldown still apply; the hazard branch falls back to spawning a Normal(2) when no Normal exists or the per-kind cap blocks the spawn.
- **Shield (Kalkan):** the player has a persistent shield resource (default 1). Tap the SHIELD HUD counter to arm it, then tap a hazard tile to cleanse it back to a Normal of the same value. Long-press the counter to drag-and-drop a shield onto a hazard tile instead. When the shield counter hits zero, tapping it opens a rewarded-video dialog; watching the full ad grants 3 more shields. Shields persist across Restart and only `Settings → Apps → Clear data` resets them.
...
- Bottom banner ad (anchored adaptive, full-width) served by Google AdMob via the Google Mobile Ads SDK. The ad slot fails gracefully on devices without Google Play Services and during dev/CI builds (which use Google's official test ad IDs).
- Rewarded-video integration (eager preload, exponential-backoff retry on load failure) used to grant 3 shields when the player taps the SHIELD counter at zero. Test ad ids in DEBUG builds; real ids only in RELEASE builds.
- Hazards (Fire / Ice / Poison) are active, persisted, and capped per board size. Spawn is **mutually exclusive**: every action either drops a Normal(2) in an empty cell OR infects an existing Normal as a hazard, never both. Phase 1 is hazard-light (9% combined hazard rate); rates ramp to fire 14% / ice 15% / poison 13% in the collapse stage. See `release-info/balancing-guide.md` for the full distribution table.
